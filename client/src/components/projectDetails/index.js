        ...
        <div className='h-[100%] bg-[#f5f5f5] fixed z-50 w-[100%] top-0 left-0'>
            ...
            <div className='grid md:grid-cols-2 overflow-scroll md:overflow-hidden h-[100vh]  md:h-[95vh]'>
                <div className='media p-4 md:p-10 md:overflow-y-scroll transperentScrollbar'>
                    ...
                    <div className='w-full h-[45vw] md:h-[20vw] overflow-hidden rounded-md mb-5 p-2  '><video src={`uploads/${project?.demoVideo}`} loading="lazy" autoPlay muted loop className='object-fill h-full w-full rounded-md' alt="" /></div>
                </div>
                ...
                <div className='details text-[3vw] md:text-[1vw] md:overflow-y-scroll transperentScrollbar  p-4 md:p-10'>
                    ...
                    <div className='p-2 gap-3 grid   '>
                        ...
                        <div className='flex gap-2'><h1 className='font-bold'>Techstacks</h1>: <p className='flex flex-wrap items-center  gap-1 lg:gap-2 gap-y-2 px-1 lg:px-2'>{Array.isArray(project?.techStacks) && project?.techStacks.length > 0 ? (
                            project?.techStacks.map((item, index) => (
                                <div key={index} className=' categories__btn px-2 lg:px-3'>{item}{!index == project?.techStacks.length - 1 && ","} </div>
                            ))
                        ) : (
